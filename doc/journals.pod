...
journals - re-DFA 项目开发日志

=head1 CONTENTS

=head2 May 15, 2006

从上周六开始，我启动了 re-DFA 项目的开发工作。该项目的目标是实现一个简单的正则
...
L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/>

有趣的是，正则表达式虽然表达能力极其强大，但是正则表达式本身作为一种语言，却无法
用自身来描述。在 re-DFA 项目中，我使用了 Parse::RecDescent 模块提供的 BNF
记法对正则表达式的语法进行了定义，该语法说明书在下面这个位置：

L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/grammar/re.grammar>
...
为 L<Parse::RecDescent> 生成的解析器是递归下降的。
...
从语法树出发我已经完成了 3 个后端 (backend)。

其中一个后端是正则表达式的反生成器 C<re::re>，它能从语法树"还原"出原始的正则表达
式。该后端可以用于测试解析器生成的语法树是否完整，同时这个 re => re 的编译器
还可以去除正则表达式中多余的括号，比如 C<((aa))*> 经过 C<re::re> 处理后得到的
是 C<(aa)*>. 有关 C<re::re> 后端的更多示例，可以从 C<re::re> 的测试脚本中得到：

L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/t/re-re/basic.t>

第二个后端是 XML 发射器，它能从语法树生成对应的 XML 描述，比如正则表达式 abc
将生成如下所示的 XML 描述：
...
最重要的后端当属 NFA 发射器了。从正则表达式生成 NFA 是生成 DFA 的一个中间步骤。
...
L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/t/re-NFA/~nfa1.png>

对于正则表达式 C<ab>，则构造出下面的 NFA 图：

L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/t/re-NFA/~nfa2.png>

正则表达式 C<a*> 是这个样子：

L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/t/re-NFA/~nfa3.png>

"重复"的 NFA 转换与老师介绍的略有不同，但从功能上讲还是完全等价的。

下一个例子是带有"替换" (alternation) 的：C<a|b>:

L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/t/re-NFA/~nfa4.png>

我们不得不承认，AT&T 的 Graphviz 库画出来的有向图真的很漂亮。

上面给出的示例都是最基本的，这儿有一个比较复杂的正则表达式，就是老师上课时用作示
例的，C<(a|b)*(aa|bb)(a|b)*>，所对应的 NFA：

L<http://svn.berlios.de/svnroot/repos/unisimu/Compilers/re-DFA/t/re-NFA/~nfa5.png>

这么大的图可能在您的 IE 浏览器中看不清楚，您可以将之下载到本地磁盘，然后用您最
喜欢的图片浏览器中查看。

考虑到 SVN 服务器有时候连接速度比较慢，我已经将上面
的 F<~nfa1.png> 至 F<~nfa5.png> 这 5 张图片放到了这封邮件的附件当中。

很自然地，从语法树得到 NFA 之后，下一步就是从 NFA 构造出 DFA 了。在发送出
这封邮件之前我就将着手这项工作，这一过程的核心是"空闭包" (eps-closure)
的构造算法，我已经想到了一种高效的实现手段了，呵呵。幸运的话，今晚就可以完成
C<re::DFA> 的实现与测试工作了。Yay!

Stay tuned!

=head2 May 16, 2006

昨晚我按计划完成了从 NFA 到 DFA 的转换模块 C<re::DFA> 的实现与测试工作。万岁！

在 re-DFA 系统中，DFA 和 NFA 使用同一种数据结构，即 C<re::Graph>
提供的基于哈希和匿名数组的有向图存储方式。由于 C<re::Graph> 类自身提供了绘制函数
C<as_png> 和 C<visualize>，因此我的 C<re::DFA> 模块发射的 DFA 便自动地拥有
了画图功能，附件中有几个例子，F<~dfa1.png> 对应于正则表达式 "C<a>"，也
对应于前一封邮件中的 F<~nfa1.png>. 其他的图片依此类推。

C<re::Graph> 类还提供了一种微型语言解释器，可以从简洁的文本描述
快速构建出有向图结构。比如附件中的 F<~dfa1.png> 可以用下面的文本来描述：
...
释器，听起来似乎有些得不偿失，
但别忘了我们使用的是 Perl 语言，C<re::Graph> 类的整个 C<build> 方法，连解
析器、执行器和出错处理在内，总共也不过 22 行代码。
...
目前，C<re::DFA> 生成的 DFA 并不是最简形式，因此需要再编写一
个 C<re::DFA::Min> 模块将 C<re::DFA> 发射的 DFA 最小
化 (minimize). 昨天晚上 C<re::DFA::Min> 模块的测试集就已经准备
好了，实现文件的头注释也早在那里了，下面就是着手编码工作了！
...
为空，即 ""，它可以匹配任何空串。同样的，正则表达式

    (a|)b*

现在也是完全合法的。附件中附上了 re-DFA 系统从该正则表达式生成
的 NFA 和 DFA 示意图。
...
目前，空字的语义在 re-DFA 的各个后端中支持很不均衡；只有 NFA/DFA 后端进行
了测试。日后显然需要添加更多这方面的测试，呵呵。

最后需要特别指出的是，附件中的那个 DFA (如 ~dfa7.png 所示）会让老师给出
的 DFA 化简算法得出错误的化简结果。（您不妨一试。）其原因在于，老师没有
对 DFA 中隐含的"出错状态"进行讨论。毕竟，在 DFA 中，许多到"出错态"的转移
都被省略掉了，但这并不是说它们总是可退到后台。
...
如果您对此有任何意见或者建议，欢迎与我进行交流与讨论。:=)

=head2 May 16, 2006

我刚刚在网上找到了一个由美国 Duke 大学开发、受美国国家科学基金资助的一
...
经过今天上午的工作，DFA 的"最小化变换"模块 C<re::DFA::Min> 终于通过了我
准备的所有的综合测试和单元测试。我看了一下，该模块的规模也不过 100 多行代码
而已，但其中却浓缩了相当复杂的集合分割算法，花费了我不少心思，呵呵。

附件中附上了综合测试中生成的最简 DFA 的示意图。这些图片与原正则表达式的对应关
系如下所示：
...
这其中有老师上课时使用的那个示例，有我们做的家庭作业，也有最基本的正则式，还有英
文版《编译原理及实践》中的一个与"出错态分割"有关的示例。

至此，从任意的正则表达式到最简 DFA 的完整通路便构建起来了：

    regex => [ re::Parser ] => regex parse tree => [ re::NFA ] => NFA
          => [ re::DFA ] => DFA => [ re::DFA::Min ] => Minimized DFA

这意味着 re-DFA 项目最初的设计目标已基本实现。下面要做的，就是完善它，扩展它，丰富它。
下面是几条主要的 TODO：
...
编写 DFA 解释器，C<re::DFA::Eval>，使 re-DFA 可以将得到的 DFA 像程序那样运行起来
...
利用 C<re::DFA::Eval> 运行 Perl 正则表达式测试集中的用例
...
re-DFA 本身是一个类库，为了向用户、尤其是非 Perl 程序员提供更加友好的接口，我在今天
晚上为 re-DFA 所有的编译器后端都编写了一个命令行工具。

=head3 re2re

首先是 re2re 程序，它解析用户提供的正则表达式，然后在内部生成一棵 Parse Tree，
...
我们看到，re2re 可以去除正则表达式中多余的括号，这也算是它的一点儿"实用价值"了。
...
然后是 re2xml 工具，它可以从用户给定的正则表达式生成其对应的语法树的 XML 描述：
...
这里我没使用很复杂的正则表达式作为示例是因为 XML 输出会变得太 FUD，呵呵。re2xml
也具有完善的出错处理。

=head3 re2nfa

re-DFA 提供的最有意思的命令行工具莫过于 re2nfa 和 re2dfa 了。我们首先来看
re2nfa。

re2nfa
根据用户提供的正则表达式生成对应的"非确定性有穷自动机"（NFA）的图形化描述：
...
最后那次调用生成的 F<DFA.png> 和 F<DFA.min.png> 已被我放到了这封邮件的附件中。我们
看到，空正则表达式的 NFA 和 DFA 其实都不为"空"，因为对于自动机而言，无论如何
都得有"初态"和"终态"。

上面介绍的 re2re, re2xml, re2nfa, 和 re2dfa 这五个命令行工具都将安装在下一个
版本的 AgentPerl 当中。请在未来几天当中关注我的个人主页

L<http://yxy.ujs.edu.cn/images/index.html>

当然了，不久的未来我还会利用 pp 和 InnoSetup 把整个 re-DFA 连同 Perl 解释器
及其依赖项一齐打包成 Win32 安装程序，这样用户就不必下载几十兆的 AgentPerl 了。

目前，re-DFA 的自动化测试台收录了 149 个测试，其中大部分是单元测试，其余为综合
测试。测试台的一次典型运行的输出如下：
    ...
    Files=8, Tests=149,  5 wallclock secs ( 0.00 cusr +  0.00 csys =  0.00 CPU)

从该报表可以看到，在我的 Pentium 4 的机器上，运行全部的测试集需要 5 秒钟的时间。
这个测试集对于我的开发工作而言至关重要，因为我是 TDD (Test-Driven Development) 的
忠实追随者、极限编程（XP）的狂热信徒，呵呵。

我现在已经开始考虑编写一个基于递归下降的语法分析器的生成器了。嗯……就类似 Damian 的
Parse::RecDescent 模块。我如果能把它的全部功能都复制出来当然就更好了。LL(k)
的分析器生成器也很有吸引力……唔，又是一个项目！

=head2 May 18, 2006

今天中午，我为 re-DFA 的 C<re::Graph> 类添加了一个叫做 normalize 的方法，
它通过对节点进行重新编号来对有向图进行"规格化" (normalization)变幻。具体的
编号方法是：从入口节点出发，利用广度优先搜索算法，逐一地对遇到的节点进行累加性
编号；而对于兄弟节点，则按其边上的权值的字母顺序，对它们进行编号。
...
所不同，从而给基于图片或数据结构精确比较的回归测试带来了不小的麻烦。
...
因此 re2dfa 在生成图片之前，总是先执行"规格化"操作。

附件中放置了经过"规格化"处理的两张 DFA 示意图，敬请欣赏。:=)

=head2 May 20, 2006

昨天，我又对 re-DFA 系统进行了功能扩展。现在 re-DFA 已能从 DFA 生成执行匹配的
Perl 代码和 C 代码了。
...
通过我的 re2c 脚本可以访问 C<re::DFA::C> 提供的这个功能。下面是一个示例：
...
我对 C<re::DFA::Perl> 和 C<re::DFA::C> 这两个 DFA 代码生成后端进行了广泛的
...
事实上，该"解释"C 代码的特性是 C<re::DFA::C> 模块通过 Ingy 的 L<Inline::C>
模块实现的。

我编写了一个名为 evalre 的命令行工具，可以"解释" DFA 的代码生成器生成的 C 代码和
Perl 代码。这样我的 re-DFA 就可以像正则表达式的计算引擎那样使用了！下面是几个示例：
...
今晚我对 evalre 工具的输出格式进行了少许修改，现在匹配成功时的输出类似下面
这个样子：
...
同时，我通过 evalre 比较了一下我们的基于 DFA 的正则表达式引擎与 Perl 自身的基于
NFA 的正则引擎在二义性处理上的区别。老师在课堂上没有提及正则表达式的"二义性"问题，比
如正则表达式 C<a|ab> 在匹配字符串 "C<ab>" 时匹配部分究竟是单个字符 "C<a>" 呢，
还是整个 "C<ab>"？
...
我已经将最新的 re-DFA 安装到了 AgentPerl 当中，您可以从我的主页下载最新
的 AgentPerl:
